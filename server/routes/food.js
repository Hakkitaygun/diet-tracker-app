  ...
  try {
    ...
    if (searchText) {
      ...
      const results = await all(
        'SELECT * FROM food_database WHERE LOWER(name) LIKE ? OR LOWER(category) LIKE ? ORDER BY name LIMIT 20',
        [`%${searchText.toLowerCase()}%`, `%${searchText.toLowerCase()}%`]
      );
...
// Generate catalog-based meal plans
